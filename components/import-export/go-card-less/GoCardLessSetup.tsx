...
import {
    ...
    getBankList,
    getRequisitionLink,
    importExpensesFromAccount,
    Bank,
    Account,
    validateToken,
    Requisition,
} from "@/helpers/GoCardLessHelper";
...
export default function GoCardLessSetup() {
    ...
    useEffect(() => {
        ...
        setupSecuredStoredValue().catch((error) =>
    ...
    useEffect(() => {
        if (isTokenValide) {
            setLoading(true);
            getBankList({ id: secretId, key: secretKey })
                .then((bankList) => {
                    setBankItems(
                        bankList.map((bank) => {
    ...
    function handleBankSelected(selectedBank: Bank) {
        setLoading(true);
        setSelectedBank(selectedBank);
        getRequisitionLink({ id: secretId, key: secretKey }, selectedBank.id)
            .then((requisition) => {
            ...
            importExpensesFromAccount({ id: secretId, key: secretKey }, selectedAccount.uuid)
                ...
                .then((expenses) => {
                    ...
                    registerBackgroundFetchAsync()
                        ...
                        .catch((error) =>
                            Alert.alert("Impossible de créer la tache d'arrère plan", `Détails : ${error}`),
                        );
                })
        ...
        <>
            ...
            <ScrollView contentContainerStyle={styles.container}>
                <View style={styles.innerContainer}>
                    ...
                    <OpenURLButton disabled={requisitionLink === ""} url={requisitionLink}>
                        Étape 3 : Autoriser l'application pour cette banque
                    </OpenURLButton>

...
function parseDate(date: string): Date {
    ...
    return new Date(
        Number.parseInt(splittedDate[2]),
        Number.parseInt(splittedDate[1]) - 1,
        Number.parseInt(splittedDate[0]),
    );
}
...
export async function createExportArchive(db: SQLiteDatabase): Promise<string> {
    ...
    if (permissionGranted) {
        let zippedFileName = (new Date().toISOString() + "export_suivi_facture")
            .replace(/[^a-z0-9]/gi, "_")
            .toLowerCase();
        ...
        return FileSystem.cpExternal(zipFilePath, zippedFileName + ".zip", "downloads").then(
            () => "downloads/" + zippedFileName,
        );
